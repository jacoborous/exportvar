...
EXPORTVAR_ROOT=$(exportvar_root)
EXPORTVAR_SCRIPTS=$(exportvar_scripts)

LICENSE=${EXPORTVAR_SCRIPTS}/license.txt
YEAR="2020"
AUTHOR="Tristan Miano"
EMAIL="jacobeus@protonmail.com"
SUBJECT="gen_template.sh"

. ${EXPORTVAR_ROOT}/shFlags/shflags
. ${EXPORTVAR_ROOT}/helper_functions.sh

DEFINE_string "filename" "" "Required. Name of BASH script to create. Location is relative to the exportvar root directory." f
...
if [ -z "${FLAGS_filename}" ] ; then
	...
	NEW_FILE="${EXPORTVAR_ROOT}/${FLAGS_filename}"
	if [ -f "${NEW_FILE}" ] ; then
...
echof "EXPORTVAR_ROOT=\$(exportvar_root)"
echof "EXPORTVAR_SCRIPTS=\$(exportvar_scripts)"
...
for i in $IMPORTS ; do echof ". \${EXPORTVAR_ROOT}/$i" ; done
